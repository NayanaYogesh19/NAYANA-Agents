...
function renderResolutionCards(resolutions) {
  ...
  resolutions.forEach((res, idx) => {
    ...
    const concerns = (comm.governance_concerns || [])
      .map((c) => `<li class="concern-item">${escapeHtml(c)}</li>`).join("");
    const paras = (comm.summary_paragraphs || [])
      .map((p) => `<p style="margin-bottom:0.6rem;color:var(--text-secondary);font-size:0.875rem;line-height:1.7;">${escapeHtml(p)}</p>`).join("");
    const risks = (comm.risk_flags || [])
      .map((r) => `<span class="badge badge-risk" style="margin-right:0.3rem;margin-bottom:0.3rem;">${escapeHtml(r)}</span>`).join("");
    ...
    card.innerHTML = `
      ...
      <div class="res-card-body">
        ${paras}
        ${concerns ? `<div class="mb-3">
          <div class="meta-label" style="margin-bottom:0.4rem;">Governance Concerns</div>
          <ul class="concern-list">${concerns}</ul>
        </div>` : ""}
        ${comm.closing_recommendation ? `<div class="closing-rec">
          ...
          <span>${escapeHtml(comm.closing_recommendation)}</span>
        </div>` : ""}
        ${risks ? `<div style="margin-top:0.75rem;">${risks}</div>` : ""}
      </div>
    `;
...
// ── Report Picker (multiple reports in one year) ──────────────────────────────
function showReportPicker(reports, company, yr) {
  // Remove any existing picker
  const existing = document.getElementById("reportPicker");
  if (existing) existing.remove();

  const picker = document.createElement("div");
  picker.id = "reportPicker";
  picker.style.cssText = `
    position:fixed;inset:0;background:rgba(10,15,30,0.7);z-index:700;
    display:flex;align-items:center;justify-content:center;
  `;
  picker.innerHTML = `
    <div style="background:var(--glass-bg,#1e2435);border:1px solid rgba(255,255,255,0.1);border-radius:1rem;padding:1.5rem;min-width:320px;max-width:480px;">
      <div style="font-size:0.85rem;font-weight:700;margin-bottom:0.25rem;">${escapeHtml(company)}</div>
      <div style="font-size:0.72rem;color:rgba(255,255,255,0.45);margin-bottom:1rem;">${yr} — ${reports.length} reports</div>
      <div style="display:flex;flex-direction:column;gap:0.5rem;" id="pickerList"></div>
      <button onclick="document.getElementById('reportPicker').remove()"
        style="margin-top:1rem;width:100%;padding:0.45rem;border-radius:0.5rem;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.5);cursor:pointer;font-size:0.78rem;">
        Cancel
      </button>
    </div>`;

  const listEl = picker.querySelector("#pickerList");
  reports.forEach((rep) => {
    const btn = document.createElement("button");
    btn.style.cssText = `
      padding:0.5rem 0.75rem;border-radius:0.5rem;background:rgba(124,58,237,0.12);
      border:1px solid rgba(124,58,237,0.3);color:#a78bfa;cursor:pointer;
      font-size:0.78rem;font-weight:600;text-align:left;
    `;
    btn.textContent = rep.label || rep.url;
    btn.addEventListener("click", () => {
      picker.remove();
      openPdfViewer(rep.url, company, `${yr} · ${rep.label}`);
    });
...
// ── PDF Viewer ────────────────────────────────────────────────────────────────
function openPdfViewer(url, title, meta) {
  const modal  = el("pdfViewerModal");
  const frame  = el("pdfViewerFrame");
  const dlBtn  = el("pdfViewerDownload");

  el("pdfViewerTitle").textContent = title;
  el("pdfViewerMeta").textContent  = meta || "";
  dlBtn.href = url;

  // Try Google Docs viewer first; if it errors fall back to direct URL
  const googleUrl = "https://docs.google.com/viewer?embedded=true&url=" + encodeURIComponent(url);
  frame.src = googleUrl;

  // If Google Docs fails to load within 8s, switch to direct URL
  let fallbackTimer = setTimeout(() => {
    if (frame.src === googleUrl) {
      frame.src = url;
    }
  }, 8000);
...
function closePdfViewer() {
  el("pdfViewerModal").style.display = "none";
  el("pdfViewerFrame").src = "";
}

// Close modal on backdrop click
el("pdfViewerModal").addEventListener("click", function(e) {
  if (e.target === this) closePdfViewer();
});

// ── Recommendation Reports sidebar ───────────────────────────────────────────
async function loadRecReports() {
  const list  = el("recReportList");
  const empty = el("recReportEmpty");
  if (!list) return;
  try {
    const res  = await fetch("/recommendation_reports");
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    list.querySelectorAll(".rec-report-item, .rec-company-group").forEach((n) => n.remove());

    if (!data.reports || data.reports.length === 0) {
      if (empty) { empty.textContent = "No recommendation reports found."; empty.style.display = ""; }
      return;
    }
    ...
    data.reports.forEach((r, idx) => {
      ...
      // Format meeting date as readable e.g. "26 Jun 2023, 2:00 PM"
      let meetingDateStr = "";
      if (r.meeting_date) {
        const dt = new Date(r.meeting_date);
        meetingDateStr = dt.toLocaleString("en-IN", {
      ...
      wrap.className = "rec-report-item";
      wrap.style.cssText = "padding:0.55rem 0.75rem;border-bottom:1px solid rgba(255,255,255,0.04);";

      const nameEl = document.createElement("div");
      nameEl.style.cssText = "font-size:0.76rem;font-weight:600;margin-bottom:0.2rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;";
      nameEl.textContent = `${idx + 1}. ${company}`;
      ...
      YEARS.forEach((yr) => {
        const raw      = r[yr];
        const dateRaw  = r[`${yr}_date`];

        let reports = [];
        if (raw) {
          reports = raw.split("\n").map(u => u.trim()).filter(Boolean).map(u => ({
            url:   u,
            label: decodeURIComponent(u.split("/").pop()).replace(/\.pdf$/i,"").replace(/InGovern.*$/i,"").trim()
          }));
        }
        const hasReports = reports.length > 0;

        // Format meeting date for this year
        let dateStr = "";
        if (dateRaw) {
        ...
        cell.style.cssText = "display:flex;flex-direction:column;gap:0.15rem;";

        const btn = document.createElement("button");
        btn.textContent = yr;
        btn.style.cssText = `
          padding:0.2rem 0.5rem;border-radius:0.35rem;font-size:0.67rem;font-weight:600;
          border:1px solid ${hasReports ? "rgba(124,58,237,0.45)" : "rgba(255,255,255,0.08)"};
          background:${hasReports ? "rgba(124,58,237,0.15)" : "transparent"};
          color:${hasReports ? "#a78bfa" : "rgba(255,255,255,0.2)"};
          cursor:${hasReports ? "pointer" : "default"};
        `;

        if (hasReports) {
          btn.title = dateStr ? `${dateStr}` : `${company} ${yr}`;
          btn.addEventListener("click", () => {
            if (reports.length === 1) {
              openPdfViewer(reports[0].url, company, `${yr} · ${dateStr || reports[0].label}`);
            } else {
              showReportPicker(reports, company, yr);
            }
          });
        }
        cell.appendChild(btn);

        // Show meeting date below the button
        if (hasReports && dateStr) {
          const dateEl = document.createElement("div");
          dateEl.style.cssText = "font-size:0.62rem;opacity:0.45;padding-left:0.1rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;";
    ...
    if (empty) empty.textContent = "Could not load reports.";
    console.error("loadRecReports:", e);
  }
}
